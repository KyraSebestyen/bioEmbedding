...
t1 = datetime.now()
a = cursor.execute(
    "SELECT token_id, embedding, year FROM embeddings INNER JOIN metadata ON embeddings.File = metadata.workId WHERE LOWER(lemma) = 'bear'").fetchall()
...
cursor.execute(f'SELECT LOWER(lemma), token_id,year FROM embeddings INNER JOIN metadata ON embeddings.File = metadata.workId WHERE LOWER(lemma) = "bear"')
...
for interval in intervals:
    ...
    ## WRITE RESULT
    result.reset_index().to_csv(f"/disk2/ksebestyen/bear_mean_pool{interval}.csv")
